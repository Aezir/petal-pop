...
- GitHub 来源安装时，播放器把分支或标签**钉死到一个 commit**，之后所有文件都从那个 commit 拉。仓库再推新内容不会悄悄改变已装的包。
- 已安装记录保存 `包id + version + commit + manifest 的 sha256`。同一个版本号被重传成不同内容也会被发现。
- "更新"按钮：重新解析来源，commit 或 manifest 哈希变了才重新安装。
...
jsDelivr 单文件最大 20MB，超过的文件拉不下来。音乐请压到 128kbps 左右（一首 4 分钟约 4MB）。
